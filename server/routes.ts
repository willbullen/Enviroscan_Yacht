...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get all ISM tasks
  apiRouter.get("/ism/tasks", async (_req: Request, res: Response) => {
    try {
      const tasks = await storage.getAllIsmTasks();
      res.json(tasks);
    } catch (error) {
      console.error("Error fetching ISM tasks:", error);
  ...
  // Get ISM tasks by category
  apiRouter.get("/ism/tasks/category/:category", async (req: Request, res: Response) => {
    try {
      const { category } = req.params;
      const tasks = await storage.getIsmTasksByCategory(category);
      res.json(tasks);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM tasks by category" });
    }
  });
  
  // Get ISM tasks by type
  apiRouter.get("/ism/tasks/type/:taskType", async (req: Request, res: Response) => {
    try {
      const { taskType } = req.params;
      const tasks = await storage.getIsmTasksByType(taskType);
      res.json(tasks);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM tasks by type" });
    }
  });
  ...
  // Get active ISM tasks
  apiRouter.get("/ism/tasks/active", async (_req: Request, res: Response) => {
    try {
      const tasks = await storage.getActiveIsmTasks();
      res.json(tasks);
    } catch (error) {
      res.status(500).json({ message: "Failed to get active ISM tasks" });
    }
  });
  ...
  apiRouter.delete("/ism/tasks/:id", async (req: Request, res: Response) => {
    try {
      ...
      const deleted = await storage.deleteIsmTask(id);
      
      if (!deleted) {
        return res.status(500).json({ message: "Failed to delete ISM task" });
      }
      ...
      await storage.createActivityLog({
        ...
        relatedEntityType: 'ism_task',
        relatedEntityId: id,
        metadata: { taskType: task.taskType, category: task.category }
      });
  ...
  // =========== ISM Task Submission Routes =============
  
  // Get all ISM task submissions
  apiRouter.get("/ism/task-submissions", async (_req: Request, res: Response) => {
    try {
      const submissions = await storage.getAllIsmTaskSubmissions();
      res.json(submissions);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM task submissions" });
    }
  });
  
  // Get ISM task submission by ID
  apiRouter.get("/ism/task-submissions/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const submission = await storage.getIsmTaskSubmission(id);
      
      if (!submission) {
        return res.status(404).json({ message: "ISM task submission not found" });
      }
      
      res.json(submission);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM task submission" });
    }
  });
  
  // Get ISM task submissions by task ID
  apiRouter.get("/ism/task-submissions/task/:taskId", async (req: Request, res: Response) => {
    try {
      const taskId = parseInt(req.params.taskId);
      const submissions = await storage.getIsmTaskSubmissionsByTask(taskId);
      res.json(submissions);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM task submissions by task" });
    }
  });
  
  // Get ISM task submissions by status
  apiRouter.get("/ism/task-submissions/status/:status", async (req: Request, res: Response) => {
    try {
      const { status } = req.params;
      const submissions = await storage.getIsmTaskSubmissionsByStatus(status);
      res.json(submissions);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM task submissions by status" });
    }
  });
  
  // Get ISM task submissions by user
  apiRouter.get("/ism/task-submissions/user/:userId", async (req: Request, res: Response) => {
    try {
      const userId = parseInt(req.params.userId);
      const submissions = await storage.getIsmTaskSubmissionsByUser(userId);
      res.json(submissions);
    } catch (error) {
      res.status(500).json({ message: "Failed to get ISM task submissions by user" });
    }
  });
  
  // Get recent ISM task submissions
  ...
  // Create new ISM task submission
  apiRouter.post("/ism/task-submissions", async (req: Request, res: Response) => {
    try {
      const validatedData = insertIsmTaskSubmissionSchema.parse(req.body);
      const submission = await storage.createIsmTaskSubmission(validatedData);
      
      // Get the task to include title in the log
      const task = await storage.getIsmTask(submission.taskId);
      const taskTitle = task ? task.title : `Task #${submission.taskId}`;
      
      // Log activity
      await storage.createActivityLog({
        activityType: 'ism_task_submitted',
        description: `New ISM task submission for: ${taskTitle}`,
        userId: submission.submittedBy || null,
        relatedEntityType: 'ism_task_submission',
      ...
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid ISM task submission data", errors: error.errors });
      }
      res.status(500).json({ message: "Failed to create ISM task submission" });
    }
  });
  
  // Update ISM task submission
  apiRouter.patch("/ism/task-submissions/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const existingSubmission = await storage.getIsmTaskSubmission(id);
      
      if (!existingSubmission) {
        return res.status(404).json({ message: "ISM task submission not found" });
      }
      
      const updatedSubmission = await storage.updateIsmTaskSubmission(id, req.body);
      
      // Log review if submission was reviewed
      if (req.body.reviewedBy && !existingSubmission.reviewedBy) {
        await storage.createActivityLog({
          activityType: 'ism_task_submission_reviewed',
          description: `ISM task submission reviewed`,
          ...
          metadata: { 
            ...
            status: req.body.status || existingSubmission.status
          }
        });
      } else {
        // Log regular update
        await storage.createActivityLog({
          activityType: 'ism_task_submission_updated',
          description: `ISM task submission updated`,
          userId: req.body.userId || null,
          relatedEntityType: 'ism_task_submission',
          relatedEntityId: id,
          metadata: { updated: Object.keys(req.body) }
      ...
      res.status(500).json({ message: "Failed to update ISM task submission" });
    }
  });
  
  // Delete ISM task submission
  apiRouter.delete("/ism/task-submissions/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const submission = await storage.getIsmTaskSubmission(id);
      ...
      const deleted = await storage.deleteIsmTaskSubmission(id);
      
      if (!deleted) {
        return res.status(500).json({ message: "Failed to delete ISM task submission" });
      }
      
      // Log activity
      await storage.createActivityLog({
        activityType: 'ism_task_submission_deleted',
        description: `ISM task submission deleted`,
        userId: req.body.userId || null,
        relatedEntityType: 'ism_task_submission',
        relatedEntityId: id,
        metadata: { 
      ...
      res.status(500).json({ message: "Failed to delete ISM task submission" });
    }
  });
