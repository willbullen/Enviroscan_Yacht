...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.get("/banking/connections/vessel/:vesselId", asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      // Get connections for this vessel
      ...
      return res.status(500).json({ error: "Failed to get banking connections for vessel" });
    }
  }));
  ...
  // Handle the case when no parameter is provided to /banking/transactions/vessel endpoint
  apiRouter.get("/banking/transactions/vessel", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required. Please log in." });
    }
    
    // This endpoint requires a vessel ID, so return an error
    return res.status(400).json({ error: "Missing vessel ID. Please use /api/banking/transactions/vessel/:vesselId" });
  }));

  // Handle the case when no parameter is provided to /banking/connections/vessel endpoint
  apiRouter.get("/banking/connections/vessel", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required. Please log in." });
    }
  ...
  // Get all reconciliations for a vessel
  apiRouter.get("/banking/reconciliation/vessel/:vesselId", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required. Please log in." });
    }
    ...
    try {
      ...
      // Optional: Get the related transactions and expenses for each reconciliation
      const detailedReconciliations = await Promise.all(
        reconciliations.map(async (reconciliation) => {
          const transaction = await storage.getBankingTransaction(reconciliation.transactionId);
          const expense = await storage.getExpense(reconciliation.expenseId);
          return {
            ...reconciliation,
            transaction,
            expense
      ...
      res.json(detailedReconciliations);
    } catch (error) {
      logger.error("Failed to get transaction reconciliations:", error instanceof Error ? error : new Error(String(error)));
      res.status(500).json({ error: "Failed to get transaction reconciliations" });
    }
  }));
  
  // Create a transaction reconciliation (match a transaction to an expense)
  apiRouter.post("/banking/reconciliation", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required. Please log in." });
    }
    
    try {
      const { transactionId, expenseId, notes } = req.body;
      
      if (!transactionId || !expenseId) {
        return res.status(400).json({ error: "Both transactionId and expenseId are required" });
      }
      
      // Check if transaction exists
      const transaction = await storage.getBankingTransaction(transactionId);
      if (!transaction) {
        return res.status(404).json({ error: "Transaction not found" });
      }
      ...
      // Check if expense belongs to same vessel as transaction
      if (transaction.vesselId !== expense.vesselId) {
        return res.status(400).json({ error: "Transaction and expense must belong to the same vessel" });
      }
      
      // Check if this transaction is already reconciled
      const existingReconciliation = await storage.getTransactionReconciliationByTransaction(transactionId);
      ...
      // Check if this expense is already reconciled
      const existingExpenseReconciliation = await storage.getTransactionReconciliationByExpense(expenseId);
      if (existingExpenseReconciliation) {
        return res.status(400).json({ 
          error: "This expense is already reconciled with another transaction",
          existingReconciliation: existingExpenseReconciliation 
        });
      }
      
      // Create the reconciliation
      const reconciliation = await storage.createTransactionReconciliation({
        transactionId,
        expenseId,
      ...
      // Log activity
      await storage.createActivityLog({
        activityType: 'transaction_reconciled',
        description: `Transaction ID ${transactionId} reconciled with Expense ID ${expenseId}`,
        userId: req.user.id,
        relatedEntityType: 'transaction_reconciliation',
        relatedEntityId: reconciliation.id,
        metadata: { 
          transactionId,
          expenseId,
      ...
      logger.error("Failed to create transaction reconciliation:", error instanceof Error ? error : new Error(String(error)));
      res.status(500).json({ error: "Failed to create transaction reconciliation" });
    }
  }));
  
  // Get unmatched transactions for a vessel
  apiRouter.get("/banking/transactions/reconciliation/:vesselId", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required. Please log in." });
    }
    ...
    try {
      // Get all transactions for the vessel
      const allTransactions = await storage.getBankingTransactionsByVessel(vesselId);
      
      // Get all reconciliations for the vessel
      const reconciliations = await storage.getTransactionReconciliations(vesselId);
      
      // Extract the IDs of reconciled transactions
      const reconciledTransactionIds = new Set(reconciliations.map(r => r.transactionId));
      
      // Filter out transactions that have already been reconciled
      const unmatchedTransactions = allTransactions.filter(tx => !reconciledTransactionIds.has(tx.id));
      ...
      logger.error("Failed to get transaction reconciliation data:", error instanceof Error ? error : new Error(String(error)));
      res.status(500).json({ error: "Failed to get transaction reconciliation data" });
    }
  }));
  
  // Delete a transaction reconciliation (unmatch a transaction and expense)
  apiRouter.delete("/banking/reconciliation/:id", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required. Please log in." });
    }
    ...
    try {
      // Get the reconciliation to be deleted (for logging)
      const reconciliation = await storage.getTransactionReconciliation(id);
      if (!reconciliation) {
        return res.status(404).json({ error: "Transaction reconciliation not found" });
      }
      
      // Get the transaction and expense for activity logging
      const transaction = await storage.getBankingTransaction(reconciliation.transactionId);
      const expense = await storage.getExpense(reconciliation.expenseId);
      
      // Delete the reconciliation
      const success = await storage.deleteTransactionReconciliation(id);
      if (!success) {
        return res.status(500).json({ error: "Failed to delete transaction reconciliation" });
      }
      
      // Log activity
      await storage.createActivityLog({
        activityType: 'transaction_unreconciled',
        description: `Transaction ID ${reconciliation.transactionId} unreconciled from Expense ID ${reconciliation.expenseId}`,
        userId: req.user.id,
        relatedEntityType: 'transaction',
        relatedEntityId: reconciliation.transactionId,
        metadata: { 
          transactionId: reconciliation.transactionId,
          expenseId: reconciliation.expenseId,
          amount: transaction ? transaction.amount : 'unknown',
          date: transaction ? transaction.transactionDate : 'unknown'
        }
      });
      
      res.status(200).json({ message: "Transaction reconciliation deleted successfully" });
    } catch (error) {
      logger.error("Failed to delete transaction reconciliation:", error instanceof Error ? error : new Error(String(error)));
      res.status(500).json({ error: "Failed to delete transaction reconciliation" });
    }
  }));
